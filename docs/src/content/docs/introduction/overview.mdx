...
**[Agentic workflows](/gh-aw/reference/glossary/#agentic-workflow)** are AI-powered repository automation that understand context, make decisions, and adapt to situations - all from natural language instructions you write in markdown.

Think of them as **intelligent GitHub Actions** powered by AI coding agents (AI systems that can write code, analyze repositories, and interact with GitHub APIs). Unlike traditional GitHub Actions with fixed if-then rules, agentic workflows use AI (like GitHub Copilot, Claude, or Codex) to:
