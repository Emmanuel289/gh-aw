...
func TestWritePromptTextToYAML_SmallText(t *testing.T) {
	...
	if strings.Count(result, `cat << 'GH_AW_PROMPT_EOF' >> "$GH_AW_PROMPT"`) != 1 {
		t.Errorf("Expected 1 heredoc block for small text, got %d", strings.Count(result, `cat << 'GH_AW_PROMPT_EOF' >> "$GH_AW_PROMPT"`))
	}
	...
	if strings.Count(result, indent+"GH_AW_PROMPT_EOF") != 1 {
		t.Errorf("Expected 1 EOF marker, got %d", strings.Count(result, indent+"GH_AW_PROMPT_EOF"))
	}
}
